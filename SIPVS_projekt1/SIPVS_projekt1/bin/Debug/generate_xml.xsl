...
<xsl:template match="objednavka">
	<html> 
		<body style="display:flex; flex-direction:column; width:20%;  margin-top: 3%; margin-left: 40%;">
		  ...
		  <h4 style="margin:0 auto;">Meno: <xsl:value-of select="zakaznik/meno"/>, Priezvisko: <xsl:value-of select="zakaznik/priezvisko"/></h4>
		  <table border="1">
			<tr bgcolor="#8abeff">	
		  ...
		  <xsl:choose>
			...
			<xsl:otherwise>
				<xsl:value-of select="zakaznik/ma_kupon"/>
				<h5>Zákazník nemá kupón!!!</h5>
			</xsl:otherwise>
		  </xsl:choose> 
		</body>
	</html>
